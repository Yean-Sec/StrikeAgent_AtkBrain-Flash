...
产出 = 尽可能多的独立 `report_finding`（低/中/高危/严重都进漏洞页，各附最小可复现 PoC）。高危/严重必须二次验证且 evidence 含已证实危害。只打到「角色不存在 / 参数不完整 / 空 data」的未授权口仍要报，**不能评高危或严重**。
...
## 评级（四级，按已证明影响对号入座；禁止抬级或压级）

info 仅二次复验失败收口，不是第四级。
...
本表只决定级别，不是收洞白名单。表里没写到的可利用问题仍必须 `report_finding`（反射 XSS、SSRF、XXE、CORS 带凭证、竞态等）：按已证明影响就近落到中危或低危，禁止因为对不上条目就丢掉。
...
入口已确认时 `web-exploit` / `src-hunt` 与 recon **同一回合并行** 开。禁止先写完 info 再打洞。新端口/路径至少测→证一轮。静态 SPA 不是无攻击面。利用 payload 被 WAF/403/406 拦截页拦住时读 skill `waf-bypass-methodology`（路径 401/403 仍走 `kali-kit` bypass-403）。
...
反射/存储/DOM。存储型默认中危；高交互、利用难度大则低危。可直接盗号且已证明大量敏感/命令执行再上调。每处独立报，不要合并。委派 `web-exploit` 或 `src-hunt`。

## 2. SQL 注入

联合/报错/盲注。先证明库名/用户名（中危）；能拖大量敏感数据或叠命令执行 → 高危；已批量读出或已执行 → 严重。时间盲注逐字节是最后手段。只用 SELECT/布尔/报错，禁止 sqlmap `--os-shell` / `--file-del`。

## 3. 命令执行 / 4. 代码执行

参数拼接、反序列化、SSTI、模板、表达式、上传解析。打成执行（canary + 回显）评严重；仅命令执行面未打成则按高危。**不要转后渗收工**，继续挖下一类。RCE 只写无害短 txt canary 并取回，立刻删掉。

## 5. 文件包含 / 6. 任意文件操作

LFI/RFI、任意读/写/删/下载。默认中危；读到大量 PII 评严重或高危（已批量读出=严重）；可写 webshell 且已解析执行=严重。禁止删业务数据。

## 7. 权限绕过

水平/垂直越权、认证绕过进后台、改他人资料、代操作。默认中危。能进后台并拿到大量敏感数据再报高危；已批量读出评严重。

## 8. 逻辑漏洞

支付改价：已证明可改金额=严重，仅逻辑面=高危。密码重置未大量泄露=中危。短信轰炸/无限流=低危。后台弱口令=中危，前台弱口令不报。支付/改价只证明参数进逻辑，不要提交落地改价。

## 9. 信息泄露

DB 连接密码/源码备份=中危；云钥匙或大量 PII=高危（已批量读出=严重）；版本/路径/phpinfo、错误页回显内网 IP 不报。heapdump 默认低危。SSRF 打到内网敏感面再报（报完切下一类，不当跳板扩网）。

## 10. 后门

预留口令、隐藏管理入口、已知 webshell、调试后门。已进系统拿权限评严重；能进后台但未拿大量数据按中危/高危对号入座。
...
- **不限轮次**；墙钟满 6 小时硬停记失败。连续 10 轮无高质量进展暂停可再开。入口连不上不暂停。
