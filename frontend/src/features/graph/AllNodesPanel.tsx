...
import { graphNodeDisplayType, graphNodeTypeLabel, nodeTypeLabel, scrubCandidateRceLabel } from "../../theme";
...
export function AllNodesPanel({ graph, onSelect }: { graph: Graph; onSelect: (node: GraphNode) => void }) {
  ...
  return <div className="nodes-panel">
    ...
    <div className="nodes-list">{nodes.map((node) => <button key={node.key} className="node-row" onClick={() => onSelect(node)}><span className={`node-type-dot n-${graphNodeDisplayType(node)}`} /><span><b>{scrubCandidateRceLabel(node.title) || node.title}</b><small>{node.key} · risk {node.risk_score}</small></span><em>{graphNodeTypeLabel(node)}</em></button>)}</div>
  </div>;
}
