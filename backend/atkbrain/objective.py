...
from .graph.rating_rubric import RATING_RUBRIC_BRIEF
...
只碰到业务报错的口不能评高危或严重，但仍要报进漏洞页。
phpinfo/指纹/Banner 不算漏洞。
1. XSS  2. SQL注入  3. 命令执行  4. 代码执行  5. 文件包含  6. 任意文件操作
7. 权限绕过  8. 逻辑漏洞  9. 信息泄露  10. 存在后门  11. N-day（已知组件版本）。
""" + RATING_RUBRIC_BRIEF + """
未授权只打到「角色不存在 / 参数不完整 / 空 data / ObjectId 报错」不能评高危或严重——打出非空业务数据，或真实领取/兑换/改价，或拖库/命令执行才能评高危/严重。
...
不限轮次；墙钟满 6 小时硬停记失败（不是未完成）。连续 10 轮无高质量进展暂停可再开。入口连不上不暂停。
